...
export const createResource = async (
  ...
  const result = await pool.query(
    `INSERT INTO resources (type, content, author, link, description, status)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING *`,
    [data.type, data.content, data.author, data.link, data.description, data.status]
  );
...
export const updateResource = async (
  ...
  const result = await pool.query(
    `UPDATE resources SET ${setQuery}, updated_at = NOW() WHERE id = $1 RETURNING *`,
    [id, ...values]
  );
